          ...
          <Container customStyles="min-h-screen py-16">
            <div className="xl:w-2/3  flex flex-col gap-10 ">
              ...
              <div className="flex gap-5 items-center">
                {userData?.img ? (
                  <div className={classNames(imageBox, 'overflow-hidden')}>
                    <Image
                      src={userData?.img}
                      width={imageSize}
                      height={imageSize}
                  ...
                  <Image
                    ...
                    height={imageSize}
                    alt="User account image"
                    className="rounded-full  border-primary-blue border-2"
                  />
                )}
              ...
              <div className={boxStyles}>
                ...
                <AccountBox
                  ...
                  value={
                    userData?.tasks?.filter((task) => task.status === Status.Completed)?.length
                  }
                />
              </div>
                  ...
                  userData.members
                    ...
                    .map((member: Members, index: number) => {
                      const finishedTasks = userData.tasks?.filter(
                        (task) =>
                          task.status === Status.Completed && task.assigned_to === member.id,
                      );
